...
import { Star, Edit, Trash, ThumbsUp, Shield, Calendar, MessageCircle, CheckCircle, Truck, UserCheck } from "lucide-react"
...
export function ReviewCard({ review, onEdit, onDelete, canModify = false }: ReviewCardProps) {
  // Function to get service delivery status badge
      ...
      <Card className="transition-all duration-300 border-0 shadow-sm hover:shadow-xl hover:shadow-blue-100 dark:hover:shadow-blue-900/20 bg-gradient-to-br from-card via-card to-card/95 backdrop-blur-sm">
          ...
          <div className="flex justify-between items-start">
            <div className="space-y-2 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className="font-semibold text-base md:text-lg text-foreground group-hover:text-primary transition-colors duration-200">
                  {review.serviceName}
                </h3>
                {review.verified && (
                  <Badge variant="secondary" className="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300 text-xs">
              ...
              <p className="text-muted-foreground font-medium text-sm md:text-base">{review.providerName}</p>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <div className="flex items-center gap-1">
                  ...
                  <span className="ml-1 md:ml-2 text-xs md:text-sm font-medium text-muted-foreground">
                    {review.rating}.0
                  </span>
                </div>
        ...
        <CardContent className="px-4 md:px-6 pb-4 md:pb-6">
          <div className="space-y-3 md:space-y-4">
            {/* Review Comment */}
            <motion.p 
              className="text-foreground/90 leading-relaxed text-sm md:text-base group-hover:text-foreground transition-colors duration-200"
              initial={{ opacity: 0.9 }}
            ...
            {review.images && review.images.length > 0 && (
              <div className="space-y-2 md:space-y-3">
                <h4 className="text-xs md:text-sm font-medium text-muted-foreground flex items-center gap-1 md:gap-2">
            ...
            {review.responseFromProvider && (
              <motion.div 
                className="mt-3 md:mt-4 p-3 md:p-4 bg-muted/30 rounded-lg border-l-4 border-l-primary/60"
            ...
            {(review.deliveryDate || review.confirmationDate) && (
              <div className="mt-3 md:mt-4 p-3 md:p-4 bg-muted/30 rounded-lg">
                <h4 className="text-xs md:text-sm font-medium text-muted-foreground mb-2">Service Timeline</h4>
                <div className="space-y-2">
                  {review.deliveryDate && (
                    <div className="flex items-center gap-1 md:gap-2 text-xs md:text-sm">
                      <Truck className="h-3 w-3 md:h-4 md:w-4 text-purple-500" />
                      <span>Delivered on {format(new Date(review.deliveryDate), "MMM d, yyyy")}</span>
                    </div>
                  )}
                  {review.confirmationDate && (
                    <div className="flex items-center gap-1 md:gap-2 text-xs md:text-sm">
                      <CheckCircle className="h-3 w-3 md:h-4 md:w-4 text-green-500" />
                      <span>Confirmed on {format(new Date(review.confirmationDate), "MMM d, yyyy")}</span>
                    </div>
                  )}
                </div>
              </div>
                  ...
                  <div className="flex items-center gap-1 md:gap-2">
                    <motion.div whileHover="hover" whileTap="tap" variants={buttonVariants}>
                      <Button 
                        variant="ghost" 
                        size="sm" 
                        className="text-muted-foreground hover:text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20 transition-colors duration-200 h-8 px-2 md:h-9 md:px-3 text-xs md:text-sm"
                      >
