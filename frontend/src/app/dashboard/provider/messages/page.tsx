...
export default function ProviderMessagesPage() {
  ...
  const activeToday = conversations.filter(conv => {
    if (!conv.last_message || !conv.last_message.timestamp) return false
    ...
    const loadConversations = async () => {
      try {
        ...
        const conversationsWithPinStatus = (response.results || []).map((conv: any) => {
          // For provider dashboard, the other participant is always the customer
          const otherParticipant = conv.customer
          const lastMessage = conv.latest_messages?.[0] || conv.last_message || null
            ...
            other_participant: {
              ...
              name: otherParticipant?.full_name ||
                    (otherParticipant?.first_name && otherParticipant?.last_name
                      ...
                      : otherParticipant?.email?.split('@')[0] || 'Customer'),
              avatar: otherParticipant?.avatar || otherParticipant?.profile_picture || null,
              is_provider: false,
            },
            service: {
  ...
  const handleRefresh = async () => {
    ...
    const loadConversations = async () => {
      try {
        setIsLoading(true)
        const response = await messagingApi.getConversations()
        const conversationsWithPinStatus = (response.results || []).map(conv => ({
          ...conv,
  ...
  const handleUnarchive = async (conversationId: number) => {
    try {
      ...
      if (item) {
        ...
        // Fallback refresh for consistency
        try {
    ...
    <div className="w-full py-8 px-4 lg:px-6 xl:px-8">
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <motion.div
            className="p-4 bg-gradient-to-r from-purple-500/5 to-pink-500/5 rounded-xl border border-purple-500/20"
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
