        ...
        >
          {isLoading ? (
            ...
            <>
              ...
              Reschedule - {service.currency} {totalPrice.toLocaleString()}
...
export default function RescheduleBookingPage({ params }: { params: Promise<{ bookingId: string }> }) {
  ...
  const RESCHEDULE_REASONS = [
    "Schedule conflict - unexpected meeting",
    "Personal emergency",
    "Weather conditions",
  ...
  const handleRescheduleSubmit = useCallback(async (formData: BookingFormData) => {
    if (!isAuthenticated) {
      showToast.warning({
        title: "Login Required",
        description: "Please login to reschedule this booking",
        duration: 4000,
        action: {
          label: "Login Now",
    ...
    if (!selectedReason || (selectedReason === "Other" && !rescheduleReason.trim())) {
      showToast.error({
        title: "Reschedule Reason Required",
        description: "Please select a reason for rescheduling your booking.",
        duration: 5000
      })
      return
    }
    ...
    try {
      ...
      // Reschedule booking with proper error handling
      let rescheduleResult: any = null
      try {
        rescheduleResult = await customerApi.rescheduleBooking(
          parseInt(resolvedParams.bookingId), 
          parseInt(selectedSlot.id), 
          rescheduleReason.trim(),
          formData.special_instructions?.trim() || ''
        )
      } catch (apiError: any) {
        console.error('Reschedule API call failed:', apiError)
        throw apiError
      }
       
      showToast.success({
        title: "Booking Rescheduled Successfully!",
        description: "Your booking has been rescheduled to the new date and time.",
        duration: 3000
      })
  ...
  }, [selectedSlot, service, isAuthenticated, router, rescheduleReason, resolvedParams.bookingId])
  ...
  useEffect(() => {
    const fetchRescheduleData = async () => {
      if (!resolvedParams.bookingId || resolvedParams.bookingId === 'undefined' || resolvedParams.bookingId === 'null') {
      ...
      try {
        ...
        const bookingData = await bookingsApi.getBookingById(parseInt(resolvedParams.bookingId))
        ...
        const serviceId = typeof bookingData.service === 'object' ? bookingData.service.id : bookingData.service
        if (!serviceId) {
          throw new Error('Service ID not found in booking data')
        }
        
        // Fetch service data
        const serviceData = await servicesApi.getServiceById(serviceId.toString())
        ...
        setFormData({
          ...
          is_express: bookingData.is_express_booking || false,
          express_type: bookingData.express_type || 'standard'
        })
        ...
        if (err.response?.status === 404) {
          setError('Booking or service not found')
        } else {
    ...
    fetchRescheduleData()
  }, [resolvedParams.bookingId, transformServiceData])
            ...
            <div className="flex items-center gap-4">
              <div className="hidden md:block">
                <Button 
                  variant="ghost" 
                  onClick={() => router.back()}
                  className="text-slate-600 hover:text-primary dark:text-slate-400 dark:hover:text-primary-foreground hover:bg-primary/10 dark:hover:bg-primary/20 transition-all duration-200 font-medium px-3 py-2 rounded-lg border border-transparent hover:border-primary/20 dark:hover:border-primary/30 hover:shadow-md"
                >
                  <ArrowLeft className="h-4 w-4 mr-2" />
                  Back to Bookings
                </Button>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {currentBooking && (
          <div className="max-w-6xl mx-auto mb-8">
            <Card className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/20 dark:to-indigo-950/20 border-blue-200/50 dark:border-blue-700/50">
              <CardHeader>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <Label className="text-sm font-medium text-slate-600 dark:text-slate-400">Current Date</Label>
                    <p className="text-lg font-semibold text-slate-900 dark:text-white">
                      {format(new Date(currentBooking.booking_date), 'EEEE, MMMM do, yyyy')}
                    </p>
                  </div>
                  <div>
                    <Label className="text-sm font-medium text-slate-600 dark:text-slate-400">Current Time</Label>
                    <p className="text-lg font-semibold text-slate-900 dark:text-white">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-semibold text-slate-900 dark:text-white">
                      NPR {parseFloat(currentBooking.total_amount).toLocaleString()}
                    </p>
                  </div>
                </div>
              </CardContent>
                    ...
                    <Button
                      ...
                      className={`justify-start h-auto p-4 text-left transition-all duration-200 ${
                        selectedReason === reason 
                          ? "bg-primary text-primary-foreground shadow-md border-primary hover:bg-primary-hover" 
                          : "hover:bg-primary/5 hover:border-primary/30 hover:text-primary dark:hover:bg-primary/10 dark:hover:border-primary/40 dark:hover:text-primary-foreground hover:shadow-sm"
                      }`}
                      ...
                      <div className="flex items-center gap-3">
                        <div className={`w-4 h-4 rounded-full border-2 transition-all duration-200 ${
                          selectedReason === reason 
                            ...
                            : "border-slate-300 dark:border-slate-600 hover:border-primary/50 dark:hover:border-primary/40"
                        }`}>
                          {selectedReason === reason && (
                            <div className="w-2 h-2 bg-primary-foreground rounded-full m-0.5 animate-scale-in" />
                          )}
                        </div>
        ...
        <div className="max-w-6xl mx-auto">
          <EnhancedBookingForm
            ...
            onBookingSubmit={handleRescheduleSubmit}
